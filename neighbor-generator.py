...
data_file_week = open('neighbor-week.csv', 'w', newline="")
csv_writer_week = csv.writer(data_file_week)
header_week = ['districtid','weekid','neighbormean','neighborstdev']
...
data_file_month = open('neighbor-month.csv', 'w', newline="")
csv_writer_month = csv.writer(data_file_month)

header_month = ['districtid','monthid','neighbormean','neighborstdev']
csv_writer_month.writerow([header_month[0], header_month[1], header_month[2],header_month[3]])

data_file_year = open('neighbor-overall.csv', 'w', newline="")
csv_writer_year = csv.writer(data_file_year)
header_year = ['districtid','overallid','neighbormean','neighborstdev']
...
#edge_data=pd.DataFrame(pd.read_csv("edge-graph.csv"))
with open('neighbor-districts-modified.json')as file:
    json_data=json.load(file)
district_id=pd.DataFrame(pd.read_csv('modified_neighbor_districts.csv'))

for distid in district_id['districtid']:
    for week_id in range(1,int(end_date_weeknum)-int(start_date_weeknum)+2):
        list_week_cases=[]
        #print(json_data[district_id.loc[district_id['districtid']==distid,'districtname'].iloc[0]][0]['neighbor'])
        for neighbor in json_data[district_id.loc[district_id['districtid']==distid,'districtname'].iloc[0]][0]['neighbor']:
            neighborid=district_id.loc[district_id['districtname']==neighbor,'districtid'].iloc[0]
            cases=int(dictionary[neighborid][week_id])
            list_week_cases.append(cases)

        #print(distid," ",week_id," ",list_week_cases)
    ...
    for month_id in range(1,int(end_date_monthnum)-int(start_date_monthnum)+2):
        ...
        #for neighborid in edge_data.loc[edge_data['districtid'] == distid,'neighbor_districtid']:
                #print(neighborid)
            cases=int(dictionary_month[neighborid][month_id])
            list_month_cases.append(cases)
                #print(list_week_cases)
                #list_week_cases.append(data_weekwise.loc[data_weekwise['weekid'] == week_id,data_weekwise['districtid']==neighborid, 'cases'].iloc[0])
        #print(distid," ",week_id," ",list_week_cases)
    ...
    #for neighborid in edge_data.loc[edge_data['districtid'] == distid, 'neighbor_districtid']:
        # print(neighborid)
        neighborid = district_id.loc[district_id['districtname'] == neighbor, 'districtid'].iloc[0]
        list_overall_cases.append(int(data_overall[((data_overall['overallid'] == 1) & (data_overall['districtid'] == neighborid))]['cases']))
        # print(list_week_cases)
        # list_week_cases.append(data_weekwise.loc[data_weekwise['weekid'] == week_id,data_weekwise['districtid']==neighborid, 'cases'].iloc[0])
    # print(distid," ",week_id," ",list_week_cases)
